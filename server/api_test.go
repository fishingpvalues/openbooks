...
func TestHealthHandler(t *testing.T) {
	for _, persist := range []bool{true, false} {
		...
		if out.Name != "openbooks" || out.Version != "5.4.3" || out.Persist != p {
			t.Errorf("persist=%v: health = %+v, want {openbooks 5.4.3 %v}", p, out, p)
		}
...
func TestPublicRouteBoundaries(t *testing.T) {
	...
	if spec.Info.Version != "5.4.3" {
		t.Errorf("openapi version = %q, want 5.4.3", spec.Info.Version)
	}
...
func TestMetricsHandler(t *testing.T) {
	...
	for _, line := range []string{
		"openbooks_up 1",
		"openbooks_version{version=\"5.4.3\"} 1",
